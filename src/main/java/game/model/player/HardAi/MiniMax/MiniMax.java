...
public class MiniMax {
  ...
  public static Turn calculateNextHardMoveMiniMax(GameState gameState, Player player) {
    ...
    for (Turn t : possibleMoves) {
      //for every possible turn are the future steps been calculated and then the best is chosen
      int turnVal;
      if (gameState.getPlayerList().size() == 2) {
      //  with two players the real minimax algorithm can be used for the evaluation of the turn
      ...
      //  2); // start with the next color
        turnVal = minimax2Player(gameState.tryTurn(t),
            gameState.getNextColor(gameState.getColorFromPlayer(player)),
            0, false, alpha, beta, 2);

      } else {
        //with four players a variation of the minimax algorithm
        // can be used for the evaluation of the turn
  ...
  public static int minimax2Player(GameState gameState, Color c, int depth, boolean isMax,
    ...
    if (isMax) { //calculation for the max player with the goal to maximize the best integer
      ...
      for (Turn t : possibleMoves) {
        //for every possible turn, the result of the next player will be calculated
        int val = Math.max(best,
            minimax2Player(gameState.tryTurn(t), gameState.getNextColor(c),
                depth + 1, false, alpha, beta, h));
        //max of the already calculated evaluation and the current evaluation with the played turn
   ...
   * @return returns the evaluation, which subtracts the score of the opponents from the other color
   */
  ...
  public static int minimax4Player(GameState gameState, Color c, int depth, int h) {
    if (depth == h) {
      return evaluate(gameState.getBoard(), c);
      //after the defined steps into the future the
      // evaluation of the board for the current color will be returned
    }
    Color current = c;
    ...
    for (Turn t : gameState.getBoard()
        .getPossibleMoves(gameState.getRemainingPolys(gameState.getPlayerFromColor(c)),
            gameState.isFirstRound())) {
      //for every possible turn, another round will be calculated
      best = Math.max(best, minimax4Player(gameState.tryTurn(t),
              gameState.getNextColor(current), depth + 1, h));
    }
