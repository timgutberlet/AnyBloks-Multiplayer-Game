...
public class MiniMax {
  ...
  public static Turn calculateNextHardMoveMiniMax(GameState gameState, Player player) {
    ...
    for (Turn t : possibleMoves) {
      //for every possible turn are the future
      // steps been calculated and then the best is chosen
      int turnVal;
      if (gameState.getPlayerList().size() == 2) {
        ...
        turnVal = minimax4Player(gameState.tryTurn(t), gameState.getColorFromPlayer(player),
            0, 2); // always with the color of the current player
      }
      if (turnVal > bestVal) {
        bestTurn = t;
        bestVal = turnVal;
  ...
  public static int minimax4Player(GameState gameState, Color c, int depth, int h) {
    if (depth == h) {
      return evaluate(gameState.getBoard(),
          c);
      //after the defined steps into the future the evaluation
      // of the board for the current color will be returned
    }
    Color current = c;
