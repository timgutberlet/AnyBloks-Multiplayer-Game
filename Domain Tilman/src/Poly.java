...
public class Poly {
  ...
  /**
   * represents the shape of the polygon as an array of boolean whether the position is filled with the polygon or not
   */
  ...
  static{
    ...
    boolean[][] s4 = new boolean[][] {{true,true,true}, {true, false, false},{true,false,false}};
    shapeList.add(s4);
    shapeList.add(s1);
    shapeList.add(s2);
    shapeList.add(s3);
  ...
  public String toString(){
    ...
    String element = "";
    switch(getColor()){
      case RED: element= "\uD83D\uDFE5"; break;
      case BLUE: element=  "\uD83D\uDFE6"; break;
      case GREEN: element=  "\uD83D\uDFE9"; break;
      case YELLOW: element=  "\uD83D\uDFE8"; break;
    }
    for(int i = 0; i < width; i++){
      for (int j = 0; j < height; j++){
        if (shape[i][j]){
