...
public class Board implements Serializable, Cloneable {
  ...
  /**
   * Method that gives back, if a specific square on the board has a side by side neighbor square of a specific color
  ...
  /**
   * Method that gives back, if a specific square on the board has a neighbor over the edge of a specific color
   * @param n row of the square
   * @param m column of the square
   * @param color searched color
  ...
  public boolean isPolyPossible(int n, int m, Poly poly, boolean isFirstRound){
    ...
    if (isFirstRound){
      if (poly.getColor().equals(Color.RED)) {
        ...
        return poly.getShape()[poly.getWidth()-1][poly.getHeight()-1] && n == getSize()-poly.getWidth()-2 && m == getSize()-poly.getHeight()-2;
      }
