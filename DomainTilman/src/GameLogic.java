...
public class GameLogic {
  ...
  /**
   * Method that gives back a list of all the possible squares, that are over the edge to already placed polygons
  ...
  private ArrayList<int[]> possibleSquares(Color color){
    ...
    for (int i = 0; i < gameState.getBoard().getSize(); i++){
      for (int j = 0; j < gameState.getBoard().getSize(); j++){
        if ( !gameState.getBoard().isColorDirectNeighbor(i,j,color) && gameState.getBoard().isColorIndirectNeighbor(i,j,color)){
          res.add(new int[]{i, j});
        }
      }
    }
   ...
   * @return returns a List with all the possible moves. This class contains position, rotation and if the polygon has to be mirrored.
   */
  ...
  /**
   * this method gives back a list of the possible positions and the specific placement of possible placements of a given polygon represented by a list of turns
   ...
   * @return list turns which contain the poly and a tuple out of integers: {row, column, rotation, mirrored}
   */
  ...
  /**
   * method that counts and safes the number of squares which could lead to a next turn for a different color, which are covered by the given turn
   * @param turn the considered turn
   * as a result the number is stored as an attribute of the turn itself
   */
      ...
      if (!c.equals(turn.getPoly().getColor())){
        for (int i = 0; i < turn.getPoly().getWidth(); i++ ){
          for (int j = 0; j < turn.getPoly().getHeight(); j++){
            if(turn.getPoly().getShape()[i][j] && gameState.getBoard().isColorIndirectNeighbor(turn.getColumn()+i, turn.getRow()+j, c)){
              num++;
            }
          }
        }
  ...
  public boolean playTurn(Turn turn){
    ...
    if(gameState.getBoard().isPolyPossible(turn.getColumn(), turn.getRow(), turn.getPoly(),gameState.isFirstRound())){
      System.out.println("Poly possible");
      for (int i = 0; i < turn.getPoly().getWidth(); i++ ){
        for (int j = 0; j < turn.getPoly().getHeight(); j++){
          if(turn.getPoly().getShape()[i][j]){
            gameState.getBoard().getBoard()[turn.getColumn()+i][turn.getRow()+j].setColor(turn.getPoly()
                .getColor());
          }
        }
      }
      ...
      for (Poly p : gameState.getRemainingPolys(gameState.getPlayerFromColor(turn.getPoly().getColor()))){
        if (p.equals(turn.getPoly())){
          gameState.getRemainingPolys(gameState.getPlayerFromColor(turn.getPoly().getColor())).remove(p);
  ...
  public static boolean playTurn(Turn turn, Board b, boolean isFirstTurn){
    ...
    if(b.isPolyPossible(turn.getColumn(), turn.getRow(), turn.getPoly(), isFirstTurn)){
      for (int i = 0; i < turn.getPoly().getWidth(); i++ ){
        for (int j = 0; j < turn.getPoly().getHeight(); j++){
          if(turn.getPoly().getShape()[i][j]){
            b.getBoard()[turn.getColumn()+i][turn.getRow()+j].setColor(turn.getPoly().getColor());
          }
        }
      }
      return true;
  ...
  public void assignRoomDiscovery(Turn turn){
    int occWidth = occupiedWidth(turn.getPoly().getColor(), gameState.getBoard(), getPlayerFromColor(turn.getPoly().getColor()).getStartX());
    int occHeight = occupiedHeight(turn.getPoly().getColor(), gameState.getBoard(), getPlayerFromColor(turn.getPoly().getColor()).getStartY());
    ...
    int occWidthAfterTurn = occupiedWidth(turn.getPoly().getColor(), boardAfterTurn, getPlayerFromColor(turn.getPoly().getColor()).getStartX());
    int occHeightAfterTurn = occupiedHeight(turn.getPoly().getColor(), boardAfterTurn, getPlayerFromColor(turn.getPoly().getColor()).getStartY());
