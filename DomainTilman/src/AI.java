...
public class AI {
  ...
  public static Turn calculateNextMove(Player player, GameLogic gameLogic){
    switch (player.getType()){
      case AI_EASY: return calculateNextEasyMove(player, gameLogic);
      case AI_MIDDLE: return calculateNextMiddleMove(player, gameLogic);
      case AI_HARD: return calculateNextHardMove(player, gameLogic);
      default: System.out.println("AI move for human player");
  ...
  public static Turn calculateNextHardMove(Player player, GameLogic gameLogic){
    ...
    int currentRoomDiscovery = GameLogic.occupiedHeight(gameLogic.getColorFromPlayer(player),gameLogic.getGameState().getBoard(), player.getStartX())
                              +GameLogic.occupiedHeight(gameLogic.getColorFromPlayer(player),gameLogic.getGameState().getBoard(), player.getStartY());

    if (currentRoomDiscovery > gameLogic.getGameState().getBoard().getSize()) { //If more than the half of the game board is discovered, it concentrates on playing against the enemy
      possibleMoves.sort(new Comparator<Turn>() {
        public int compare(Turn o1, Turn o2) {
          return o1.getNumberBlockedSquares() - o2.getNumberBlockedSquares();
        }
